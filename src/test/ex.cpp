...
static void TestMessage(bool WantedSys, int WantedID, const void *pData, int Size, CUnpacker *pUnpacker)
{
	...
	CUuid Uuid;
	CMsgPacker Answer(0);
	EXPECT_EQ(UnpackMessageID(&ID, &Sys, &Uuid, pUnpacker, &Answer, g_pConfig), (int)UNPACKMESSAGE_OK);
...
static void GetWhatIsAnswer(int Uuid, CMsgPacker *pPacker)
{
	...
	bool Sys;
	CUuid TmpUuid;
	ASSERT_EQ(UnpackMessageID(&ID, &Sys, &TmpUuid, &Unpacker, pPacker, g_pConfig), (int)UNPACKMESSAGE_ANSWER);
	EXPECT_EQ(Sys, true);
